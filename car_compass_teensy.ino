...
*/
float declinationAngle = -0.13;//hazel park
...
int addrPins[4] = {0, 1, 2, 3};
int dataPins[8] = {8, 9, 10, 11, 12, 13, 14, 15};
int resetPin = 4; //active low
int flashPin = 5; //active low
int writePin = 6; //active low
int enablePin = 7; //active low
int brightness = 7;
boolean flashing = 0;
boolean clears = 0;
boolean blinking = 0;
boolean lampTest = 0;
char cmd[] = {'C', 'o', 'm', 'p', 'a', 's', 's', ' '};
char bli[] = {0, 0, 0, 0, 0, 0, 0, 0};
...
void loop()
{
 ...
 float directionDegrees = 360 - (direction * 180/pi);
  
 String dirString = cardinal.getString(type, directionDegrees);
  ...
  Serial.println(dirString);   // heading string (N, NE, E, etc...)
      ...
      brightness = map(analogRead(A9),1,1023,7,0);
      setCommand();

switch (cardinal.getInteger(type, directionDegrees)) {
  case 1:
  cmd[0] = 'N';
  cmd[1] = ' ';
  break;
  ...
  cmd[2] = (temperature<0.0)?'-':' ';
  cmd[3] = int(temperature)/10+0x30;
  cmd[4] = int(temperature)%10+0x30;
  cmd[5] = '.';
  cmd[6] = int(10*(temperature - int(temperature)))+0x30;  
  cmd[7] = 'C';
  //bli[0] = 1;
  writeDisp();
